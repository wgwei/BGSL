...
import numpy as np
cimport numpy as np
def intersection(double x1, double y1, double x2, double y2,double x3, double y3, double x4, double y4):
...
def is_interSegment(double x, double y, double x1, double y1, double x2, double y2,double x3, double y3, double x4, double y4):
    ''' Check wether point is the intersection point of line [lineStartPt, lineEndPt]
        ...
        lineEndPt2 = [x, y]
    '''
    cdef double leftPtLi, rightPtLi, highPtLi, lowPtLi, leftPtL2, rightPtL2, highPtL2, lowPtL2, xlimMin, xlimMax, ylimMin, ylimMax
    leftPtLi = min(x1, x2)
    rightPtLi = max(x1, x2)
    highPtLi = max(y1, y2)
    lowPtLi = min(y1, y2)
...
def line_x_poly(double lineStartPtx, double lineStartPty, double lineEndPtx, double lineEndPty,\
             np.ndarray polygonx, np.ndarray polygony):
